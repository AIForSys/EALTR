...
class LTR:
    ...
    def Objfunction(self, Param):
        ...
        if (self.costflag=='loc'):
            ...
            performancevalue= Origin_PerformanceMeasure(real_y, pred_y, self.cost,self.cost, 0.2, 'density', 'loc').getSomePerformance()[14]
            ...
            pred_y=pred_y * np.array(self.cost)
            real_y = self.training_data_y * np.array(self.cost)
            performancevalue = PerformanceMeasure(real_y, pred_y, self.cost,self.cost, 1, 'complexitydensity','cc').POPT()
...
if __name__ == '__main__':
    ...
    for dataset, filename in Processing().import_single_data():
        ...
        de = LTR(X=training_data_X, y=training_data_y, cost=cost, costflag='loc',logorlinear='linear')
        de.process()